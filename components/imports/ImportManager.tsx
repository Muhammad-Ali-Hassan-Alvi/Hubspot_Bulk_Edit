...
import { Upload, FileSpreadsheet, Download, CheckCircle, Loader2, AlertTriangle } from 'lucide-react'
  ...
  useEffect(() => {
    if (contentTypesOptions.length > 0 && !contentTypesLoading) {
      ...
      if (!isValidOption) {
        ...
        const defaultOption = contentTypesOptions.find(option => option.value === 'landing-pages') || contentTypesOptions[0]
        ...
        <div className="flex items-center justify-between">
          <div>
            <CardTitle className="flex items-center gap-2">
              <Download className="h-5 w-5" /> Import {contentTypesOptions.find(opt => opt.value === selectedContentType)?.label || selectedContentType}
            </CardTitle>
            <CardDescription>
          ...
          <div className="flex items-center gap-4">
            <Select value={selectedContentType} onValueChange={handleContentTypeChange}>
              <SelectTrigger className="w-[200px]">
                <SelectValue placeholder={contentTypesLoading ? "Loading..." : "Select content type"} />
              </SelectTrigger>
              <SelectContent>
                {contentTypesOptions.length > 0 ? (
                  contentTypesOptions.map(contentTypeOption => (
              ...
              <div className="space-y-4">
                <div className="flex items-center gap-2 p-3 bg-yellow-50 border border-yellow-200 rounded-lg">
                  ...
                  <span className="text-sm text-yellow-800">
                    <strong>Note:</strong> Only CSV files exported from this system are supported. Convert your Excel file to CSV format first. The file should be in the format: hubspot_{selectedContentType.toLowerCase().replace(/\s+/g, '_')}_X_items_YYYY-MM-DD.csv
                  </span>
                </div>
