...
export async function POST(request: NextRequest) {
  try {
    ...
    console.log(`✅ Fetched: ${domains.length} domains, ${campaigns.length} campaigns, ${authors.length} authors, ${contentGroups.length} content groups, ${tags.length} tags, ${topics.length} topics, ${redirects.length} redirects`)
    ...
    if (contentType === 'all-pages') {
      ...
      allContent = await fetchAllFromEndpoint(contentEndpoints[contentType as keyof typeof contentEndpoints], headers)
    }
    ...
    // 3. Authors
    dropdownOptions.authorName = authors.map(author => author.displayName || author.name).filter(Boolean)
    ...
    // 11. Redirect options
    dropdownOptions.redirectStyle = redirects.map(redirect => redirect.redirectStyle).filter(Boolean)
    dropdownOptions.precedence = redirects.map(redirect => redirect.precedence?.toString()).filter(Boolean)

    // Cache the results
    dropdownCache.set(cacheKey, { data: dropdownOptions, timestamp: Date.now() })

    console.log(`🎉 Successfully processed ${Object.keys(dropdownOptions).length} dropdown categories`)
