...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    if (searchError) {
      return NextResponse.json(
        { error: 'Failed to search for existing users' },
        { status: 500 }
      )
    }
    ...
    const hasEmailLinked = user.email === email || 
      user.identities?.some(identity => 
        identity.provider === 'email' && 
        identity.identity_data?.email === email
      )
    ...
    if (linkedEmails.length >= 1) {
      return NextResponse.json(
        { error: 'You can only link ONE additional email account. Please unlink the existing one first.' },
        { status: 400 }
      )
    }
    ...
    try {
      ...
      return NextResponse.json({
        success: true,
        message: 'Email account linked successfully. You now have your primary email and one linked email.',
        linkedEmail: email,
...
export async function DELETE(request: NextRequest) {
  try {
    ...
    const remainingIdentities = user.user.identities?.filter(identity => 
      !(identity.provider === 'email' && identity.identity_data?.email === email)
    ) || []
    ...
    if (searchError) {
      return NextResponse.json(
        { error: 'Failed to find linked account' },
        { status: 500 }
      )
    }

    const linkedUser = linkedUsers.users.find(u => 
      u.email === email && 
      u.user_metadata?.linked_to_user_id === userId
    )
