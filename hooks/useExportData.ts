...
export const useExportData = () => {
  ...
  const loadAllRecordsForContentType = useCallback(async (contentType: string, totalCount: number) => {
    try {
      await dispatch(fetchAllRecordsForContentType({ contentType, totalCount })).unwrap()
    } catch (error) {
  ...
  const getContentCount = useCallback((contentType: string) => {
  ...
  const getStoredRecords = useCallback((contentType: string) => {
  ...
  const isContentTypeComplete = useCallback((contentType: string) => {
    return exportData.contentTypeData[contentType]?.isComplete || false
  }, [exportData.contentTypeData])

  // Get paginated records (500 per page)
  const getPaginatedRecords = useCallback((contentType: string, page: number = 1, pageSize: number = 500) => {
  ...
  const clearContentType = useCallback((contentType: string) => {
